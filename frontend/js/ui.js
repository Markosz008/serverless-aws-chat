...
export function addMessage(text, isSystem = false, sender = '', isMine = false, msgId = '', replyTo = null, linkPreview = null, status = 'sent', timestamp = 0, initialReactions = null, audioUrl = null) {
    ...
    // --- ÚJ: KALANDMESTER AUDIO LEJÁTSZÓ ÉS ANIMÁCIÓ ---
    if (audioUrl) {
    ...
    if (!isSystem && msgId && status !== 'pending' && status !== 'failed') {
        ...
        hoverBtn.onclick = (e) => { e.stopPropagation(); if(window.openReactMenu) window.openReactMenu(e.clientX, e.clientY, msgId, sender || state.myUsername, text); }; msgDiv.appendChild(hoverBtn);
        
        const replyBtn = document.createElement('div'); replyBtn.className = 'desktop-reply-btn'; replyBtn.innerHTML = '↩️'; 
        replyBtn.onclick = (e) => { e.stopPropagation(); if(window.initiateReply) window.initiateReply(sender || state.myUsername, text); }; msgDiv.appendChild(replyBtn);
    }
